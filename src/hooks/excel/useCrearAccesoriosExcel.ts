...
import { insertarAccesoriosExcelService } from "@/services/excel";

export function useCrearAccesoriosExcel() {
  ...
  const uploadExcelAccesorios = async (file: File) => {
    ...
    try {
      const response = await insertarAccesoriosExcelService(file);
