...
const SingleGridItemProduct = ({ item }: { item: any }) => {
  ...
  const handleAddToCart = () => {
    // Normalizamos los datos antes de enviarlos a Redux
    const itemToCart = {
      id: item.id,
    ...
    <div className="group">
      <div className="relative overflow-hidden flex items-center justify-center rounded-lg bg-white shadow-1 min-h-[270px] mb-4">
        ...
        <div className="absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-2.5 pb-5 ease-linear duration-200 group-hover:translate-y-0">
          {/* Si es LENTE: Solo botón de Matriz */}
          {isLente ? (
            <Link
              href={{
                pathname: "/matrix",
            ...
            /* Si es FRAME: Iconos del Template */
            <>
              {/* Botón Ojo (Quick View) */}
              <button
                onClick={() => {
                  openModal();
      ...
      <div className="flex items-center gap-2.5 mb-2">
        ...
        {item.reviews && <p className="text-custom-sm">({item.reviews})</p>}
      </div>
      ...
      <div className="flex flex-col gap-1">
        {isLente ? (
          <div className="flex flex-wrap gap-2 text-custom-sm font-medium">
            <span className="text-blue bg-blue/5 px-2 rounded">
              S1: S/. {item.precio_serie1}
            </span>
            <span className="text-blue bg-blue/5 px-2 rounded">
              S2: S/. {item.precio_serie2}
            </span>
            <span className="text-blue bg-blue/5 px-2 rounded">
              S3: S/. {item.precio_serie3}
            </span>
          </div>
        ) : (
          <span className="flex items-center gap-2 font-medium text-lg">
            <span className="text-dark">
              S/. {item.producto?.precio || item.discountedPrice}
            </span>
            {item.price && (
