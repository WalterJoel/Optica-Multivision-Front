...
import { useMonturasExcel, useEditarMonturasExcel } from "@/hooks/excel";
import { STATUS_MODAL } from "@/commons/constants";
import { useSessionUser } from "@/hooks/session";
import { MONTURA_EDITAR_COLUMNS, aplicarValidacionesMonturasExcel } from "../validacionesExcel";

export const DropEditarMonturas = () => {
  ...
  const { uploadExcelMonturas, loading, statusMessage, success } =
    useEditarMonturasExcel();

  const { obtenerMonturasExcel } = useMonturasExcel();
  ...
  // Evitar doble click al descargar
  const handleDownload = async () => {
    ...
    try {
      const data = await obtenerMonturasExcel(sedeId);

      if (!data?.length) return;

      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet("Monturas");

      worksheet.columns = MONTURA_EDITAR_COLUMNS;
      ...
      aplicarValidacionesMonturasExcel(worksheet, maxRows);
      ...
      anchor.href = url;
      anchor.download = `monturas_sede_${sedeId}.xlsx`;
      anchor.click();
    ...
    <div className="bg-white border border-gray-100 rounded-[28px] shadow-testimonial p-5 sm:p-6 h-full flex flex-col">
      ...
      <div className="flex items-center gap-4 mb-5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <h2 className="text-base font-bold text-dark">Editar Monturas</h2>
        </div>

        {/* BOTÓN DESCARGA BLOQUEADO */}
        <BaseButton
          fullWidth={false}
        ...
        >
          ...
          {downloading ? "Generando..." : "Descargar Formato"}
        </BaseButton>
      </div>
