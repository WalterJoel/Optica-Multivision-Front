...
import { useCrearAccesoriosExcel } from "@/hooks/excel";
...
export const DropCrearAccesorios = () => {
  ...
  // Hooks
  const { uploadExcelAccesorios, loading, statusMessage, success } =
    useCrearAccesoriosExcel();
  ...
  const handleUpload = async () => {
    ...
    try {
      await uploadExcelAccesorios(file);
      setFile(null);
    } catch (error) {
      console.error(error);
    }
    ...
    <div className="bg-white border border-gray-100 rounded-[28px] shadow-testimonial p-5 sm:p-6 h-full flex flex-col">
      ...
      <div className="flex items-center gap-4 mb-5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <h2 className="text-base font-bold text-dark">Crear Accesorios</h2>
        </div>
      ...
      <div className="mb-5 p-4 rounded-2xl bg-yellow-light-4 border border-yellow-light-2 text-yellow-dark-2 text-xs flex gap-3 items-start shadow-sm">
        ...
        <div>
          <span className="font-bold">Nota importante:</span> Recuerda que al crear accesorios, se considerará la cantidad únicamente para la sede actual y para el resto de sedes será cero.
        </div>
      </div>
